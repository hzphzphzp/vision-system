...
class ImageStitchingTool(ToolBase):
        ...
        if matcher_type == "FLANN":
            try:
                if detector_type in ["SIFT", "SURF"]:
                    ...
                    index_params = dict(algorithm=FLANN_INDEX_KDTREE, trees=8)  # 增加树数量，提升匹配精度
    ...
    def process(self, input_data: List[ImageData]) -> ResultData:
        ...
        try:
            ...
            self._logger.info(f"图像尺寸统计: 宽度范围={min(widths)}-{max(widths)}, 高度范围={min(heights)}-{max(heights)}, 平均尺寸={avg_width}x{avg_height}")
            ...
            self._logger.info(f"特征点检测完成: 有效特征={len(valid_features)}/{len(features)}")
            ...
            if stitched_image.is_valid and stitched_image.width > 0 and stitched_image.height > 0:
                ...
                self._logger.info(f"拼接成功! 输出尺寸: {stitched_image.width}x{stitched_image.height}, 处理时间: {processing_time:.2f}秒")
            else:
                # 如果拼接失败，返回第一张图像
                result.status = False
    ...
    def preprocess_image(self, image: np.ndarray) -> np.ndarray:
        ...
        # 自适应直方图均衡化（比普通均衡化更自然）
        clahe = cv2.createCLAHE(clipLimit=1.5, tileGridSize=(16, 16))  # 降低clipLimit，避免过曝
    ...
    def _match_features(self, features1: Dict[str, Any], features2: Dict[str, Any]) -> List:
        ...
        try:
            ...
            matches1 = self._matcher.knnMatch(
                desc1, 
                desc2, 
                k=2
            )
            ...
            # 如果方向1的匹配点足够多，直接返回
            if len(good_matches1) > 30:
                self._logger.info(f"方向1匹配点充足，直接返回: {len(good_matches1)}个匹配点")
                return good_matches1
            
            # 方向2：features2 → features1
            matches2 = self._matcher.knnMatch(
                desc2, 
                desc1, 
                k=2
            )
            ...
            if len(mutual_matches) < 10:
                self._logger.info(f"互匹配点不足，使用单向匹配结果")
                # 选择匹配点更多的方向
                if len(good_matches1) >= len(good_matches2):
                    ...
                    for m in good_matches2:
                        # 创建一个新的匹配点，模拟方向1→方向2的格式
                        class Match:
                            def __init__(self, queryIdx, trainIdx, distance):
                                self.queryIdx = queryIdx
                                self.trainIdx = trainIdx
                                self.distance = distance
                        # 注意：这里只是为了保持格式一致，实际使用时需要确保特征点对应正确
                        # 由于方向2是desc2→desc1，我们需要交换索引
                        final_matches.append(Match(m.trainIdx, m.queryIdx, m.distance))
            else:
                final_matches = mutual_matches
            
            self._logger.info(f"匹配结果: 方向1良好匹配数={len(good_matches1)}, 方向2良好匹配数={len(good_matches2)}, 互匹配数={len(mutual_matches)}, 最终匹配数={len(final_matches)}")
    ...
    def _sort_images(self, features: List[Dict[str, Any]]) -> List[int]:
        ...
        def kruskal_mst(num_nodes, similarity_matrix):
            ...
            for i in range(num_nodes):
                for j in range(i + 1, num_nodes):
                    ...
                    edges.append((-weight, i, j))  # 使用负权重将最大生成树问题转换为最小生成树问题
        ...
        def dfs_order(start_node, adj_list):
            ...
            def dfs(node):
                if node not in visited:
                    ...
                    order.append(node)
                    # 按相似度排序邻居，优先访问相似度高的
                    neighbors = sorted(adj_list[node], key=lambda x: -x[1])
        ...
        start_idx = np.argmax(total_similarity)
    ...
    def check_mirror(self, img1: np.ndarray, img2: np.ndarray) -> bool:
        ...
        try:
            ...
            self._logger.info(f"镜像检测：原始相似度={sim_original:.3f}, 翻转后={sim_flip:.3f}")
            # 翻转判定：翻转后相似度更高且>0.2
            return sim_flip > sim_original and sim_flip > 0.2
        except Exception as e:
    ...
    def _stitch_images(self, images: List[ImageData], features: List[Dict[str, Any]]) -> ImageData:
        ...
        self._logger.info(f"初始化拼接结果尺寸: {result_image.width}x{result_image.height}")
        ...
        for i in range(1, len(images)):
            ...
            self._logger.info(f"特征点匹配数: {len(matches)}, 最小要求: {min_match_count}")
            ...
            try:
                ...
                dst_pts = np.float32([current_features["keypoints"][m.trainIdx].pt for m in matches])
                ...
                if M1 is not None:
                    inliers1 = mask1.sum()
                    self._logger.info(f"方向1（第二张图向第一张图对齐）内点数量={inliers1}/{len(matches)}")
                    best_M = M1
                    best_inliers = inliers1
                
                if M2 is not None:
                    inliers2 = mask2.sum()
                    self._logger.info(f"方向2（第一张图向第二张图对齐）内点数量={inliers2}/{len(matches)}")
                ...
                self._logger.info(f"选择最佳方向{direction}，内点数量={best_inliers}/{len(matches)}")
                M = best_M
                ...
                self._logger.info(f"放置区域: x[{x_start},{x_end}], y[{y_start},{y_end}]")
                ...
                if y_start < y_end and x_start < x_end:
                    ...
                    if copy_h > 0 and copy_w > 0:
                        result_warped[y_start:y_start+copy_h, x_start:x_start+copy_w] = result_image.data[:copy_h, :copy_w]
                    else:
                    ...
                    if default_y_end > default_y_start and default_x_end > default_x_start:
                        self._logger.info(f"使用默认位置放置图像: x[{default_x_start},{default_x_end}], y[{default_y_start},{default_y_end}]")
                        copy_h = default_y_end - default_y_start
                        copy_w = default_x_end - default_x_start
                        result_warped[default_y_start:default_y_end, default_x_start:default_x_end] = result_image.data[:copy_h, :copy_w]
                    else:
                ...
                self._logger.info(f"融合后尺寸: {stitched.shape[1]}x{stitched.shape[0]}")
                ...
                # 检查裁剪后的尺寸
                if stitched_cropped.size > 0 and stitched_cropped.shape[0] > 0 and stitched_cropped.shape[1] > 0:
                    stitched_final = stitched_cropped
                    self._logger.info(f"裁剪后尺寸: {stitched_final.shape[1]}x{stitched_final.shape[0]}")
                else:
                    # 裁剪失败，使用原始融合结果
                    stitched_final = stitched
                    self._logger.warning(f"裁剪失败，使用融合结果: {stitched_final.shape[1]}x{stitched_final.shape[0]}")
                
                # 更新结果图像
                self._logger.info(f"更新结果图像: {stitched_final.shape[1]}x{stitched_final.shape[0]}")
                result_image = ImageData(data=stitched_final)
                self._logger.info(f"更新后ImageData尺寸: {result_image.width}x{result_image.height}")
        ...
        if result_image.is_valid and result_image.width > 0 and result_image.height > 0:
            self._logger.info(f"拼接完成! 最终结果尺寸: {result_image.width}x{result_image.height}")
        else:
            # 检查是否至少有部分拼接成功
            success_count = 0
            for i in range(1, len(images)):
                ...
                if result_image.data.size > 0 and not np.array_equal(result_image.data, images[0].data):
                    success_count += 1
                    break
            
            if success_count > 0:
                self._logger.warning("拼接过程中有部分成功，但最终结果尺寸无效，尝试修复")
                # 尝试裁剪或调整结果图像
                if result_image.data.size > 0:
                    # 尝试裁剪黑边
                    stitched = self._crop_black_borders(result_image.data)
                    if stitched.size > 0 and stitched.shape[0] > 0 and stitched.shape[1] > 0:
                        result_image = ImageData(data=stitched)
                        self._logger.info(f"修复后结果尺寸: {result_image.width}x{result_image.height}")
                    else:
                        # 裁剪失败，保留原始拼接结果
                        ...
                        if result_image.data.size > 0:
                            ...
                            self._logger.info(f"保留原始拼接结果尺寸: {result_image.width}x{result_image.height}")
            
            # 如果仍然无效，返回第一张图像
            if not result_image.is_valid or result_image.width <= 0 or result_image.height <= 0:
    ...
    def _crop_black_borders(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            self._logger.info(f"裁剪黑边成功：从 {image.shape[1]}x{image.shape[0]} 裁剪到 {cropped.shape[1]}x{cropped.shape[0]}")
            return cropped
        except Exception as e:
            self._logger.error(f"裁剪黑边失败: {e}")
            return image
    
    def _balance_brightness(self, img1: np.ndarray, img2: np.ndarray, mask: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _blend_images(self, img1: np.ndarray, img2: np.ndarray, mask1: np.ndarray = None, mask2: np.ndarray = None) -> np.ndarray:
        """
    ...
    def _run_impl(self):
        ...
        if hasattr(self, '_input_data_list') and len(self._input_data_list) >= 2:
            ...
            self._logger.info(f"开始拼接 {len(self._input_data_list)} 张图像: {input_sizes}")
            ...
            if process_result.status and hasattr(process_result, 'get_image'):
                ...
                if stitched_image:
                    # 记录拼接结果信息
                    self._logger.info(f"拼接成功! 输入: {len(self._input_data_list)}张图像, 输出尺寸: {stitched_image.width}x{stitched_image.height}")
                    
                    self._output_data = stitched_image
                    # 清空输入数据列表，准备接收新数据
                    self._input_data_list.clear()
                    return {
                        "OutputImage": stitched_image
                    }
            else:
                # 拼接失败，记录错误信息
        ...
        # 对于单张图像，返回输入图像
        self._logger.info(f"处理单张图像: {self._input_data.width}x{self._input_data.height}")
        return {
            "OutputImage": self._input_data
        }
    ...
    def set_input(self, input_data, port_name="InputImage"):
        ...
        if input_data:
            ...
            # 记录输入数据信息
            self._logger.debug(f"添加输入图像 #{len(self._input_data_list)}: {input_data.width}x{input_data.height}")
