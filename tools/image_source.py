...
class CameraSource(ImageSourceToolBase):
    ...
    def _init_params(self):
        """初始化默认参数"""
        self.set_param("camera_id", "0", param_type="string", 
                       description="相机ID")
        self.set_param("trigger_mode", "software", param_type="enum", 
                       description="触发模式",
                       options=["software", "continuous", "hardware"])
        self.set_param("fps", 30, param_type="integer", 
                       description="帧率")
        self.set_param("exposure", 10000, param_type="integer", 
                       description="曝光时间（微秒）")
        self.set_param("gain", 0, param_type="float", 
                       description="增益")
        self.set_param("width", 640, param_type="integer", 
                       description="图像宽度")
        self.set_param("height", 480, param_type="integer", 
                       description="图像高度")
        self.set_param("auto_exposure", True, param_type="boolean", 
                       description="自动曝光")
        self.set_param("auto_gain", True, param_type="boolean", 
                       description="自动增益")
    ...
    def _run_impl(self):
        ...
        try:
            ...
            if trigger_mode == "software":
                if not self._camera.trigger_software():
                    raise Exception("软触发失败，请检查相机连接状态")
            
            # 采集一帧图像
            self._logger.info("采集一帧图像")
            image_data = self._camera.capture_frame(timeout_ms=2000)
            
            if not image_data:
                raise Exception("无法获取相机图像，请检查相机连接状态")
            
            self._logger.info(f"成功采集图像: {image_data.width}x{image_data.height}")
    ...
    def show_settings_dialog(self, parent=None, camera_settings_tool=None):
        ...
        if camera_settings_tool:
            ...
            self._camera_settings_tool = CameraParameterSettingTool("camera_settings")
        
        # 同步当前CameraSource的参数到相机参数设置工具
        self._camera_settings_tool.set_param("camera_id", self.get_param("camera_id", "0"))
        self._camera_settings_tool.set_param("exposure", self.get_param("exposure", 10000))
        self._camera_settings_tool.set_param("gain", self.get_param("gain", 0))
        self._camera_settings_tool.set_param("width", self.get_param("width", 640))
        self._camera_settings_tool.set_param("height", self.get_param("height", 480))
        self._camera_settings_tool.set_param("fps", self.get_param("fps", 30))
        self._camera_settings_tool.set_param("trigger_mode", self.get_param("trigger_mode", "continuous"))
        self._camera_settings_tool.set_param("auto_exposure", self.get_param("auto_exposure", True))
        self._camera_settings_tool.set_param("auto_gain", self.get_param("auto_gain", True))
        ...
        if result == 1:  # QDialog.Accepted
            ...
            trigger_mode = self._camera_settings_tool.get_param("trigger_mode", "continuous")
            auto_exposure = self._camera_settings_tool.get_param("auto_exposure", True)
            ...
            self._logger.info(f"更新相机参数: camera_id={camera_id}, width={width}, height={height}, fps={fps}, trigger_mode={trigger_mode}")
