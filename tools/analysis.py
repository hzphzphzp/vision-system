...
class BlobFind(VisionAlgorithmToolBase):
    ...
    def _run_impl(self):
        ...
        # 确定阈值方法
        threshold_method = self.THRESHOLD_METHODS.get(threshold_method_name, cv2.THRESH_BINARY)
...
class PixelCount(VisionAlgorithmToolBase):
    ...
    def _run_impl(self):
        ...
        # 执行阈值处理
        threshold_method = BlobFind.THRESHOLD_METHODS.get(threshold_method_name, cv2.THRESH_BINARY)
        _, binary = cv2.threshold(gray, threshold_value, 255, threshold_method)
...
class Histogram(VisionAlgorithmToolBase):
    ...
    def _run_impl(self):
        ...
        if histogram_type == "gray":
            ...
            # 计算直方图
            hist = cv2.calcHist([gray], [0], None, [hist_size], [range_min, range_max])
            ...
            for i, (channel, color) in enumerate(zip(channels, colors)):
                # 计算直方图
                hist = cv2.calcHist([channel], [0], None, [hist_size], [range_min, range_max])
                # 归一化
                hist = cv2.normalize(hist, hist).flatten()
...
class Caliper(VisionAlgorithmToolBase):
    ...
    def _run_impl(self):
        ...
        for i in range(caliper_count):
            ...
            if draw_caliper:
                # 绘制卡尺中心线
                cv2.line(output, (start_x, caliper_y), (end_x, caliper_y), (0, 255, 0), 1)
                # 绘制搜索区域
                cv2.rectangle(output, (start_x, caliper_y - search_region // 2), 
                              (end_x, caliper_y + search_region // 2), (0, 255, 0), 1)
            
            # 绘制边缘
            if draw_edges:
                for edge in edges:
                    cv2.circle(output, (edge["position"], edge["y"]), 3, (0, 0, 255), -1)
            
            # 绘制测量结果
            if draw_result and "distance" in caliper_result:
                mid_x = (start_x + end_x) // 2
                cv2.putText(output, f"{caliper_result['distance']:.1f}", 
                           (mid_x, caliper_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 
                           0.5, (255, 255, 255), 1)
        ...
        self._result_data.set_value("total_edges", sum(result["edge_count"] for result in caliper_results))
        
        self._logger.info(f"卡尺测量完成，共 {len(caliper_results)} 个卡尺，检测到 {self._result_data.get_value('total_edges')} 个边缘")
