...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QDoubleSpinBox, QComboBox, QPushButton, QCheckBox, QMessageBox,
    QGridLayout, QWidget, QLineEdit
)
from PyQt5.QtCore import Qt, QThread, pyqtSignal

from core.tool_base import ToolBase, ToolParameter, ToolPort, ToolRegistry
from modules.camera_manager import CameraManager, CameraInfo, HikCamera
...
class CameraParameterSettingTool(ToolBase):
    ...
    def _update_camera_parameters(self, camera: HikCamera):
        ...
        try:
            ...
            if not auto_exposure:
                if not self._set_camera_parameter(camera, "ExposureTime", exposure):
            ...
            if not self._set_camera_parameter(camera, "AcquisitionFrameRate", fps):
    ...
    def _set_camera_parameter(self, camera: HikCamera, param_name: str, value: Any):
        """设置相机参数"""
        try:
            ...
            # 根据参数类型设置值
            if param_name in ["ExposureTime", "Gain", "Gamma", "AcquisitionFrameRate"]:
                # 浮点参数
                try:
                ...
                ret = camera._camera.MV_CC_SetFloatValue(param_name.encode('utf-8'), stParam)
                ...
                ret = camera._camera.MV_CC_SetIntValue(param_name.encode('utf-8'), stParam)
            else:
                # 其他参数
    ...
    def _import_mv_camera_control(self):
        """导入海康相机SDK模块"""
        from modules.camera_manager import _import_mv_camera_control
...
class CameraParameterDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.auto_exposure_check.stateChanged.connect(self.update_exposure_enabled)
    ...
    def connect_camera(self):
        ...
        try:
            if self.connected_camera:
                ...
                # 断开CameraManager中的连接
                try:
                    self._logger.info(f"尝试断开CameraManager中的相机: {final_camera_id}")
                    self.camera_manager.disconnect(final_camera_id)
                    self._logger.info(f"CameraManager中的相机已断开: {final_camera_id}")
                except Exception as e:
    ...
    def apply_parameters(self):
        ...
        try:
            ...
            self.tool.set_param("auto_exposure", self.auto_exposure_check.isChecked())
            self.tool.set_param("auto_gain", self.auto_gain_check.isChecked())
            
            # 执行工具
            result = self.tool.run()
            
            if isinstance(result, dict) and result.get("Status", "").startswith("参数设置成功"):
