...
class OCRReader(RecognitionToolBase):
    ...
    PARAM_DEFINITIONS = {
        "language": ToolParameter(
            ...
            default="ch_sim",
            description="OCR识别语言",
            options=["ch_sim", "en", "ja", "ko", "ch_sim+en", "en+ch_sim", "all"]
        ),
        ...
        if cls._ocr_model is None:
            with cls._ocr_model_lock:
                if cls._ocr_model is None:
                    ...
                    try:
                        import easyocr
                        cls._ocr_model = easyocr.Reader(['ch_sim', 'en'], gpu=False)
    ...
    def _run_impl(self):
        ...
        self._logger.info(f"开始OCR识别: language={language}, min_confidence={min_confidence}")
        ...
        try:
            ...
            if len(input_image.shape) == 3:
                ...
                result = reader.readtext(cv2.cvtColor(input_image, cv2.COLOR_GRAY2BGR))
            ...
            if detailed_results:
                for i, (text, conf, bbox) in enumerate(zip(texts, 
                    ...
                    draw.text(text_pos, f"{text} ({conf:.2f})", fill=(0, 255, 0), font=font)
