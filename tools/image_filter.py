...
from core.tool_base import ImageProcessToolBase, ToolRegistry, ToolParameter
from data.image_data import ImageData, ROI
...
class BoxFilter(ImageProcessToolBase):
    ...
    def _run_impl(self):
        ...
        self._logger.info(f"方框滤波完成: kernel={kernel_size}, normalize={normalize}")
...
class GaussianFilter(ImageProcessToolBase):
    ...
    def _run_impl(self):
        ...
        self._logger.info(f"高斯滤波完成: kernel={kernel_size}, sigma_x={sigma_x}, sigma_y={sigma_y}")
...
class BilateralFilter(ImageProcessToolBase):
    ...
    def _run_impl(self):
        ...
        self._logger.info(f"双边滤波完成: diameter={diameter}, sigma_color={sigma_color}, sigma_space={sigma_space}")
...
class Morphology(ImageProcessToolBase):
    ...
    def _run_impl(self):
        ...
        # 执行形态学操作
        output = cv2.morphologyEx(input_image, operation, kernel, iterations=iterations)
        
        self._output_data = self._input_data.copy()
        self._output_data.data = output
        
        self._logger.info(f"形态学处理完成: operation={operation_name}, kernel={kernel_size}, iterations={iterations}")
...
class ImageResize(ImageProcessToolBase):
    ...
    def _init_params(self):
        ...
        self.set_param("interpolation", "linear", 
                       param_type="enum", 
    ...
    def _run_impl(self):
        ...
        # 获取插值方法
        interpolation = self.INTERPOLATION_METHODS.get(interpolation_name, cv2.INTER_LINEAR)
        
        # 执行缩放
        output = cv2.resize(input_image, (width, height), interpolation=interpolation)
        
        self._output_data = self._input_data.copy()
        self._output_data.data = output
        
        self._logger.info(f"图像缩放完成: {input_image.shape[1]}x{input_image.shape[0]} -> {width}x{height}")
