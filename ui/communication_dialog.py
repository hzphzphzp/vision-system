...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QWidget, QFormLayout, QListWidget, QListWidgetItem,
    QMessageBox, QInputDialog, QGridLayout, QFrame, QTextEdit
)
...
class CommunicationConfigDialog(QDialog):
    ...
    def _check_received_data(self):
        ...
        for protocol_name, protocol in list(self._protocol_instances.items()):
            if protocol_name not in self._connections or not self._connections[protocol_name]:
                continue
            ...
            try:
                if protocol_name == "TCP客户端":
                    ...
                    if data:
                        self._display_received_data(f"TCP服务端 [客户端{client_id}]", data)
    ...
    def _on_tab_changed(self, index):
        ...
        if not self._is_closed and hasattr(self, 'status_label') and self.status_label is not None:
            try:
                self.status_label.setText(f"当前: {self._current_protocol} - 就绪")
            except:
                pass
    ...
    def _create_tcp_client_widgets(self):
        ...
        """)
        self._connect_buttons["TCP客户端"].clicked.connect(self._toggle_tcp_client)
    ...
    def _create_tcp_server_widgets(self):
        ...
        """)
        self._connect_buttons["TCP服务端"].clicked.connect(self._toggle_tcp_server)
        ...
        """)
        self._send_test_buttons["TCP服务端"].clicked.connect(self._send_tcp_server_test)
    ...
    def _create_serial_widgets(self):
        ...
        self.serial_baudrate.addItems(["9600", "19200", "38400", "57600", "115200", "230400", "460800"])
    ...
    def _create_websocket_widgets(self):
        ...
        """)
        self._connect_buttons["WebSocket"].clicked.connect(self._toggle_websocket)
        ...
        """)
        self._send_test_buttons["WebSocket"].clicked.connect(self._send_ws_test)
    ...
    def _create_modbus_widgets(self):
        ...
        """)
        self._connect_buttons["Modbus断开"].clicked.connect(self._disconnect_modbus)
    ...
    def _create_modbus_operation_widgets(self):
        ...
        """)
        self._send_test_buttons["Modbus"].clicked.connect(self._execute_modbus_operation)
    ...
    def _connect_tcp_client(self):
        ...
        self._add_log(f"正在连接TCP客户端: {config['host']}:{config['port']}...", "INFO")
    ...
    def _start_tcp_server(self):
        ...
        self._add_log(f"正在启动TCP服务端监听: 0.0.0.0:{config['port']}...", "INFO")
        
        server = TCPServer()
        
        def on_client_connect(cid, addr):
            QTimer.singleShot(0, lambda: self._add_log(f"客户端 {cid} 已连接: {addr}", "INFO"))
        
        def on_client_disconnect(cid):
            QTimer.singleShot(0, lambda: self._add_log(f"客户端 {cid} 已断开", "INFO"))
        
        def on_receive(cid, data):
            QTimer.singleShot(0, lambda: self._add_log(f"接收 [TCP服务端]: [客户端{cid}]: {data}", "RECV"))
        
        def on_error(msg):
            QTimer.singleShot(0, lambda: self._add_log(f"错误 [TCP服务端]: {msg}", "ERROR"))
    ...
    def _open_serial(self):
        ...
        self._add_log(f"正在打开串口: {config['port']} @ {config['baudrate']}bps", "INFO")
        
        serial = SerialPort()
        
        def on_connected():
            self._connections["串口"] = True
            QTimer.singleShot(0, lambda: self._update_button_state("串口", True))
            QTimer.singleShot(0, lambda: self._update_status("串口已打开"))
            QTimer.singleShot(0, lambda: self._add_log("串口打开成功!", "SUCCESS"))
        
        def on_disconnected():
            self._connections["串口"] = False
            QTimer.singleShot(0, lambda: self._update_button_state("串口", False))
            QTimer.singleShot(0, lambda: self._update_status("串口已断开"))
            QTimer.singleShot(0, lambda: self._add_log("串口已断开", "WARNING"))
        
        def on_error(msg):
            QTimer.singleShot(0, lambda: self._add_log(f"错误 [串口]: {msg}", "ERROR"))
    ...
    def _connect_websocket(self):
        ...
        def on_connected():
            ...
            QTimer.singleShot(0, lambda: self._update_button_state("WebSocket", True))
            QTimer.singleShot(0, lambda: self._update_status("WebSocket已连接"))
            QTimer.singleShot(0, lambda: self._add_log("WebSocket连接成功!", "SUCCESS"))
        
        def on_disconnected():
            self._connections["WebSocket"] = False
            QTimer.singleShot(0, lambda: self._update_button_state("WebSocket", False))
            QTimer.singleShot(0, lambda: self._update_status("WebSocket已断开"))
            QTimer.singleShot(0, lambda: self._add_log("WebSocket已断开连接", "WARNING"))
        
        def on_message(data):
            QTimer.singleShot(0, lambda: self._add_log(f"接收 [WebSocket]: {data}", "RECV"))
        
        def on_error(msg):
            QTimer.singleShot(0, lambda: self._add_log(f"错误 [WebSocket]: {msg}", "ERROR"))
    ...
    def _toggle_http(self):
        ...
        if http.connect(config):
            ...
            if response["success"]:
                self._add_log(f"HTTP连接成功，状态码: {response['status_code']}", "SUCCESS")
            else:
                self._add_log(f"HTTP请求失败: {response.get('error', '未知错误')}", "ERROR")
            http.disconnect()
        else:
    ...
    def _connect_modbus(self):
        ...
        self._add_log(f"正在连接Modbus TCP: {config['host']}:{config['port']}...", "INFO")
        
        client = ModbusTCPClient()
        
        def on_connected():
            self._connections["Modbus"] = True
            QTimer.singleShot(0, lambda: self._update_button_state("Modbus", True))
            QTimer.singleShot(0, lambda: self._update_status("Modbus已连接"))
            QTimer.singleShot(0, lambda: self._add_log("Modbus TCP连接成功!", "SUCCESS"))
        
        def on_disconnected():
            self._connections["Modbus"] = False
            QTimer.singleShot(0, lambda: self._update_button_state("Modbus", False))
            QTimer.singleShot(0, lambda: self._update_status("Modbus已断开"))
            QTimer.singleShot(0, lambda: self._add_log("Modbus TCP已断开", "WARNING"))
        
        def on_error(msg):
            QTimer.singleShot(0, lambda: self._add_log(f"错误 [Modbus]: {msg}", "ERROR"))
    ...
    def _execute_modbus_operation(self):
        ...
        self._add_log(f"执行: {func_names[func_idx]} 地址={address}, 数量={count}", "INFO")
        ...
        try:
            if func_idx == 0:
                ...
                if success:
                    self._add_log(f"读离散输入 [{address}]: {values}", "SUCCESS")
                else:
                ...
                if success:
                    self._add_log(f"读保持寄存器 [{address}]: {values}", "SUCCESS")
                else:
                ...
                if success:
                    self._add_log(f"读输入寄存器 [{address}]: {values}", "SUCCESS")
                else:
                ...
                if success:
                    self._add_log(f"写多寄存器 [{address}]: {num}个", "SUCCESS")
                else:
    ...
    def _add_log(self, message, level="INFO"):
        ...
        if not self._is_closed and hasattr(self, 'log_text') and self.log_text is not None:
            try:
...
class CommunicationMonitorWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.protocol_combo.addItems(["TCP客户端", "TCP服务端", "串口", "WebSocket", "HTTP"])
