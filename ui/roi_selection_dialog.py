...
class ROIEditorCanvas(ZoomableFrameMixin, QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("border: 1px solid #CCCCCC; background-color: #333333;")
    ...
    def set_image(self, image):
        ...
        if isinstance(image, np.ndarray):
            if len(image.shape) == 3:
                ...
                self._image = QImage(
                    image.data, image.shape[1], image.shape[0],
                    image.shape[1], QImage.Format.Format_Grayscale8
                )
    ...
    def _validate_roi_data(self):
        ...
        if "x" in self._roi_data and "y" in self._roi_data:
            self._roi_data["x"] = max(0, min(self._roi_data["x"], img_width - 1))
            self._roi_data["y"] = max(0, min(self._roi_data["y"], img_height - 1))
        
        if "width" in self._roi_data:
            self._roi_data["width"] = max(1, min(self._roi_data["width"], img_width))
        
        if "height" in self._roi_data:
            self._roi_data["height"] = max(1, min(self._roi_data["height"], img_height))
        
        if "x1" in self._roi_data:
            self._roi_data["x1"] = max(0, min(self._roi_data["x1"], img_width - 1))
        if "y1" in self._roi_data:
            self._roi_data["y1"] = max(0, min(self._roi_data["y1"], img_height - 1))
        
        if "x2" in self._roi_data:
            self._roi_data["x2"] = max(0, min(self._roi_data["x2"], img_width - 1))
        if "y2" in self._roi_data:
            self._roi_data["y2"] = max(0, min(self._roi_data["y2"], img_height - 1))
        
        if "cx" in self._roi_data:
            self._roi_data["cx"] = max(0, min(self._roi_data["cx"], img_width - 1))
        if "cy" in self._roi_data:
            self._roi_data["cy"] = max(0, min(self._roi_data["cy"], img_height - 1))
        
        if "radius" in self._roi_data:
            self._roi_data["radius"] = max(1, min(self._roi_data["radius"], min(img_width, img_height) // 2))
    ...
    def paintEvent(self, event):
        ...
        if self._current_point and self._image and self._is_drawing:
            ...
            if x <= self._current_point.x() <= x + img_width and \
               y <= self._current_point.y() <= y + img_height:
    ...
    def _draw_roi(self, painter: QPainter, offset_x: float, offset_y: float):
        ...
        if roi_type == "rect" and all(k in self._roi_data for k in ["x", "y", "width", "height"]):
            self._draw_rect_roi(painter, offset_x, offset_y)
        elif roi_type == "line" and all(k in self._roi_data for k in ["x1", "y1", "x2", "y2"]):
            self._draw_line_roi(painter, offset_x, offset_y)
        elif roi_type == "circle" and all(k in self._roi_data for k in ["cx", "cy", "radius"]):
            self._draw_circle_roi(painter, offset_x, offset_y)
    
    def _draw_rect_roi(self, painter: QPainter, offset_x: float, offset_y: float):
        """绘制矩形ROI及控制点"""
        def to_canvas(x, y):
            return int(x * self._scale + offset_x), int(y * self._scale + offset_y)
        ...
        painter.drawText(left, top - 5, f"ROI: ({self._roi_data['x']}, {self._roi_data['y']}) {width}x{height}")
    
    def _draw_line_roi(self, painter: QPainter, offset_x: float, offset_y: float):
        """绘制直线ROI"""
        def to_canvas(x, y):
            return int(x * self._scale + offset_x), int(y * self._scale + offset_y)
        ...
        painter.drawEllipse(QPoint(x1, y1), self._handle_size, self._handle_size)
        painter.drawEllipse(QPoint(x2, y2), self._handle_size, self._handle_size)
        
        font = QFont()
        font.setPointSize(10)
        painter.setFont(font)
        painter.drawText(x1, y1 - 5, f"直线: ({self._roi_data['x1']}, {self._roi_data['y1']})")
    
    def _draw_circle_roi(self, painter: QPainter, offset_x: float, offset_y: float):
        """绘制圆形ROI"""
        def to_canvas(x, y):
            return int(x * self._scale + offset_x), int(y * self._scale + offset_y)
        ...
        painter.setBrush(QColor(0, 255, 0))
        painter.drawEllipse(QPoint(cx, cy), self._handle_size, self._handle_size)
        
        font = QFont()
        font.setPointSize(10)
        painter.setFont(font)
        painter.drawText(cx + radius + 5, cy, f"圆: r={self._roi_data['radius']}")
    
    def _draw_handles(self, painter: QPainter, x: int, y: int, width: int, height: int):
        """绘制控制点"""
        painter.setBrush(QColor(0, 255, 0))
        ...
        for hx, hy in handles:
            painter.drawEllipse(QPoint(hx, hy), self._handle_size, self._handle_size)
    
    def _get_handle_at_position(self, pos: QPoint, offset_x: float, offset_y: float) -> int:
        """获取指定位置的控制点"""
        ...
        if abs(px - left) < handle_threshold and abs(py - top) < handle_threshold:
            return self.DRAG_RESIZE_LT
        elif abs(px - (left + width)) < handle_threshold and abs(py - top) < handle_threshold:
            return self.DRAG_RESIZE_RT
        elif abs(px - left) < handle_threshold and abs(py - (top + height)) < handle_threshold:
            return self.DRAG_RESIZE_LB
        elif abs(px - (left + width)) < handle_threshold and abs(py - (top + height)) < handle_threshold:
    ...
    def _update_cursor(self, pos: QPoint, offset_x: float, offset_y: float):
        ...
        self.setCursor(cursors.get(handle, QCursor(Qt.CursorShape.ArrowCursor)))
    ...
    def mousePressEvent(self, event):
        ...
        if not (x <= event.pos().x() <= x + self._image.width() * self._scale and
                y <= event.pos().y() <= y + self._image.height() * self._scale):
            return
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._roi_type == "rect":
            ...
            radius = int(np.sqrt((img_x - start_img_x) ** 2 + (img_y - start_img_y) ** 2))
            self._roi_data = {
                "type": "circle",
                "cx": center_x,
                "cy": center_y,
...
class ROISelectionDialog(QDialog):
    ...
    def __init__(self, parent=None, title: str = "选择ROI区域", roi_type: str = "rect"):
        super().__init__(parent)
    ...
    def _init_ui(self):
        ...
        info_layout.addWidget(QLabel("拖拽移动 | 拖拽边角调整大小 | 方向键微调 | Ctrl+Z撤销 | 滚轮缩放"))
...
def show_roi_editor(parent, image, title: str = "选择ROI区域", roi_type: str = "rect", 
                   roi_data: dict = None) -> dict:
    """显示ROI编辑对话框
