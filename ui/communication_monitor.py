...
class CommunicationMonitorPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        for i, label in enumerate(
            [
                self.total_sent_label,
                self.total_received_label,
                self.active_connections_label,
                self.error_count_label,
            ]
        ):
            label.setStyleSheet(
                """
        ...
        self.protocol_combo.addItems(
            ["TCP客户端", "TCP服务端", "串口", "WebSocket", "Modbus TCP"]
        )
