...
class CPUOptimizationDialog(QDialog):
    ...
    def _create_memory_tab(self):
        ...
        try:
            ...
            info_layout.addWidget(
                QLabel(f"{mem.total / (1024**3):.2f} GB"), 0, 1
            )

            info_layout.addWidget(QLabel("已用内存:"), 1, 0)
            info_layout.addWidget(
                QLabel(f"{mem.used / (1024**3):.2f} GB ({mem.percent}%)"), 1, 1
            )

            info_layout.addWidget(QLabel("可用内存:"), 2, 0)
            info_layout.addWidget(
                QLabel(f"{mem.available / (1024**3):.2f} GB"), 2, 1
            )
        except ImportError:
    ...
    def _create_simd_tab(self):
        ...
        level_layout.addWidget(
            QLabel(f"当前优化级别: {caps.optimization_level.upper()}")
        )
    ...
    def load_current_settings(self):
        ...
        self.thread_spinner.setValue(
            int(omp_threads) if omp_threads.isdigit() else 0
        )
...
class PerformanceMonitorWidget(QWidget):
    ...
    def _update_display(self):
        ...
        try:
            ...
            # 更新统计信息
            self.total_inferences_label.setText(
                str(stats.get("total_inferences", 0))
            )
            self.avg_inference_label.setText(
                f"{stats.get('avg_inference_time', 0):.2f} ms"
            )
            self.min_inference_label.setText(
                f"{stats.get('min_inference_time', float('inf')):.2f} ms"
            )
            self.max_inference_label.setText(
                f"{stats.get('max_inference_time', 0):.2f} ms"
            )
