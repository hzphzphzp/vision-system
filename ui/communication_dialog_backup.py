...
class DisconnectWorker(QThread):
    ...
    def run(self):
        """在后台线程执行断开操作"""
        logger.info("[DisconnectWorker] 后台线程开始执行断开操作")
        try:
            # 复制协议实例列表，避免在迭代过程中修改
            protocols = list(self._dialog._protocol_instances.items())
            logger.info(f"[DisconnectWorker] 需要断开 {len(protocols)} 个协议")
            
            for protocol_name, protocol in protocols:
                try:
                    logger.info(f"[DisconnectWorker] 断开 {protocol_name}...")
                    if protocol:
                        # 清除回调
                        if hasattr(protocol, 'clear_callbacks'):
                            try:
                                ...
                                logger.warning(f"[DisconnectWorker] 清除 {protocol_name} 回调失败: {e}")
                        ...
                        if protocol_name == "TCP服务端":
                            try:
                                ...
                                logger.warning(f"[DisconnectWorker] 停止 {protocol_name} 失败: {e}")
                        else:
                            try:
                                protocol.disconnect()
                            except Exception as e:
                                logger.warning(f"[DisconnectWorker] 断开 {protocol_name} 失败: {e}")
                        
                        logger.info(f"[DisconnectWorker] {protocol_name} 处理完成")
                except Exception as e:
                    logger.error(f"[DisconnectWorker] 处理 {protocol_name} 时出错: {e}")
            
            # 清理字典
            self._dialog._protocol_instances.clear()
            self._dialog._connections.clear()
            logger.info("[DisconnectWorker] 所有协议已断开，字典已清理")
            
        except Exception as e:
            logger.error(f"[DisconnectWorker] 后台断开操作失败: {e}")
        finally:
            logger.info("[DisconnectWorker] 后台线程完成")
            self.finished.emit()
...
class CommunicationConfigDialog(QDialog):
    ...
    def _disconnect_all(self):
        """断开所有连接"""
        logger.info(f"[CommunicationDialog] 开始断开所有连接，当前协议实例: {list(self._protocol_instances.keys())}")
        ...
        for protocol_name in list(self._protocol_instances.keys()):
            try:
                ...
                if protocol:
                    # 清除回调防止内存泄漏
                    if hasattr(protocol, 'clear_callbacks'):
                        logger.debug(f"[CommunicationDialog] 清除 {protocol_name} 的回调")
                        protocol.clear_callbacks()
                    # 断开连接
                    if protocol_name == "TCP服务端":
                        logger.debug(f"[CommunicationDialog] 调用 {protocol_name}.stop()")
                        protocol.stop()
                    else:
                        logger.debug(f"[CommunicationDialog] 调用 {protocol_name}.disconnect()")
                        protocol.disconnect()
                    logger.info(f"[CommunicationDialog] {protocol_name} 已断开")
            except Exception as e:
                logger.error(f"[CommunicationDialog] 断开 {protocol_name} 时出错: {e}")
    ...
    def _stop_tcp_server(self):
        ...
        if "TCP服务端" in self._protocol_instances:
            try:
                protocol = self._protocol_instances["TCP服务端"]
                if hasattr(protocol, 'clear_callbacks'):
                    protocol.clear_callbacks()
                protocol.stop()
            except Exception as e:
    ...
    def _close_serial(self):
        ...
        if "串口" in self._protocol_instances:
            try:
                protocol = self._protocol_instances["串口"]
                if hasattr(protocol, 'clear_callbacks'):
                    protocol.clear_callbacks()
                protocol.disconnect()
            except Exception as e:
    ...
    def _disconnect_websocket(self):
        ...
        if "WebSocket" in self._protocol_instances:
            try:
                protocol = self._protocol_instances["WebSocket"]
                if hasattr(protocol, 'clear_callbacks'):
                    protocol.clear_callbacks()
                protocol.disconnect()
            except Exception as e:
    ...
    def _disconnect_modbus(self):
        ...
        if "Modbus" in self._protocol_instances:
            try:
                protocol = self._protocol_instances["Modbus"]
                if hasattr(protocol, 'clear_callbacks'):
                    protocol.clear_callbacks()
                protocol.disconnect()
            except Exception as e:
    ...
    def closeEvent(self, event):
        """关闭事件"""
        logger.info("[CommunicationDialog] closeEvent 被调用")
        self._is_closed = True

        # 先停止定时器
        if self._receive_timer:
            logger.info("[CommunicationDialog] 停止接收定时器")
            self._receive_timer.stop()
            self._receive_timer = None

        # 使用后台线程断开所有连接，避免阻塞UI
        logger.info("[CommunicationDialog] 启动后台线程断开连接...")
        self._disconnect_worker = DisconnectWorker(self)
        self._disconnect_worker.finished.connect(self._on_disconnect_finished)
        self._disconnect_worker.start()
        
        # 忽略原始事件，等待后台线程完成后再关闭
        event.ignore()
        logger.info("[CommunicationDialog] closeEvent 完成，等待后台线程...")
    
    def _on_disconnect_finished(self):
        """后台断开操作完成回调"""
        logger.info("[CommunicationDialog] 后台断开操作完成，关闭对话框")
