...
class ProjectTreeItem(QTreeWidgetItem):
    ...
    def __init__(
        self, item_type: ItemType, name: str, item_obj: Any = None, parent=None
    ):
...
class ProjectBrowserWidget(QWidget):
    ...
    item_renamed = pyqtSignal(
        str, object, str
    )  # item_type, item_object, new_name
    ...
    def _refresh_tree(self):
        """刷新项目树"""
        self._logger.info(
            f"刷新项目树 - 流程数量: {len(self._solution.procedures) if self._solution else 0}"
        )
        ...
        # 创建方案根节点
        solution_item = ProjectTreeItem(
            ItemType.SOLUTION, self._solution.name, self._solution
        )
        solution_item.setText(1, "方案")
        self.project_tree.addTopLevelItem(solution_item)

        # 创建流程节点
        procedures_item = ProjectTreeItem(
            ItemType.FOLDER, "流程", None, solution_item
        )
        ...
        for i, procedure in enumerate(self._solution.procedures):
            self._logger.info(
                f"  流程[{i}]: {procedure.name}, 工具数量: {procedure.tool_count}"
            )
            proc_item = ProjectTreeItem(
                ItemType.PROCEDURE, procedure.name, procedure, procedures_item
            )
            ...
            for j, tool in enumerate(procedure.tools):
                self._logger.info(
                    f"    工具[{j}]: {tool.name} ({tool.tool_name})"
                )
                tool_item = ProjectTreeItem(
                    ItemType.TOOL, tool.name, tool, proc_item
                )
    ...
    def _on_rename(self, item: ProjectTreeItem):
        ...
        new_name, ok = QInputDialog.getText(
            self, "重命名", f"输入{item.type.value}的新名称:", text=item.name
        )
        if ok and new_name:
            old_name = item.name
            item.set_name(new_name)
            self._logger.info(
                f"重命名{item.type.value}: {old_name} -> {new_name}"
            )
    ...
    def _on_delete_item(self, item: ProjectTreeItem):
        ...
        if not item or not item.isSelected():
            self._logger.debug(
                f"节点已在前置处理中被删除: {item.name if item else 'unknown'}"
            )
            return
        ...
        if parent:
            ...
            self.project_tree.takeTopLevelItem(
                self.project_tree.indexOfTopLevelItem(item)
            )
    ...
    def _on_new_procedure(self):
        ...
        proc_name, ok = QInputDialog.getText(
            self,
            "新建流程",
            "输入流程名称:",
            text=f"流程{len(self._solution.procedures) + 1}",
        )
        if ok and proc_name:
            from core.procedure import Procedure
...
class ProjectBrowserDockWidget(QDockWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 连接信号
        self.project_browser.item_double_clicked.connect(
