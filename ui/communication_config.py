...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QLabel, QGroupBox, QGridLayout, QSplitter,
    QComboBox, QLineEdit, QSpinBox, QTextEdit, QTabWidget,
...
from core.communication.protocol_base import ProtocolBase, DataParser
...
class ProtocolFactory:
    ...
    @staticmethod
    def create_protocol(protocol_type: str, config: Dict[str, Any]) -> Optional[ProtocolBase]:
        """创建协议实例
        ...
        try:
            if protocol_type == "TCP客户端":
                ...
                # WebSocket协议实现
                from core.communication.websocket_protocol import WebSocketProtocol
                ...
                # Modbus TCP协议实现
                from core.communication.modbus_tcp_protocol import ModbusTCPProtocol
        ...
        try:
            ...
            for conn in connections:
                conn_data = {
                    ...
                    "last_modified": getattr(conn, 'last_modified', time.time()),
    ...
    def load_connections(self) -> List[ProtocolConnection]:
        ...
        try:
            ...
            for conn_data in data:
                ...
                conn.last_modified = conn_data.get('last_modified', time.time())
...
class ConnectionMonitor:
    ...
    def monitor_connection(self, connection: ProtocolConnection) -> bool:
        ...
        if not connection.is_connected:
            # 增加错误计数
            self._alerts[connection.id] = self._alerts.get(connection.id, 0) + 1
            
            # 检查是否达到报警阈值
            if self._alerts[connection.id] >= self._alert_threshold:
                logger.warning(f"连接 {connection.name} 异常，已连续失败 {self._alerts[connection.id]} 次")
                return True
        else:
            # 重置错误计数
...
class ConnectionManager:
    ...
    def add_connection(self, connection: ProtocolConnection) -> bool:
        ...
        with self._lock:
            # 验证配置
            valid, error_msg = self._validator.validate_config(connection.protocol_type, connection.config)
            if not valid:
                logger.error(f"添加连接失败: {error_msg}")
                return False
            
            # 创建协议实例
            connection.protocol_instance = ProtocolFactory.create_protocol(connection.protocol_type, connection.config)
    ...
    def update_connection(self, connection_id: str, config: Dict[str, Any]) -> bool:
        ...
        with self._lock:
            ...
            # 验证配置
            valid, error_msg = self._validator.validate_config(conn.protocol_type, config)
            if not valid:
            ...
            # 创建新的协议实例
            conn.protocol_instance = ProtocolFactory.create_protocol(conn.protocol_type, config)
    ...
    def start_connection(self, connection_id: str) -> bool:
        ...
        with self._lock:
            ...
            if not conn.protocol_instance:
                conn.protocol_instance = ProtocolFactory.create_protocol(conn.protocol_type, conn.config)
                if not conn.protocol_instance:
                    logger.error(f"创建协议实例失败: {conn.protocol_type}")
                    return False
            else:
    ...
    def test_connection(self, connection_id: str) -> Tuple[bool, str]:
        ...
        with self._lock:
            ...
            # 创建临时协议实例进行测试
            temp_instance = ProtocolFactory.create_protocol(conn.protocol_type, conn.config)
    ...
    def get_active_connections(self) -> List[ProtocolConnection]:
        """获取活跃连接"""
        with self._lock:
            return [conn for conn in self._connections.values() if conn.is_connected]
    ...
    def _update_connection_status(self):
        ...
        for conn in self._connections.values():
            if conn.protocol_instance and hasattr(conn.protocol_instance, 'is_connected'):
                ...
                # 通知状态变化
                if self._status_callback and (conn.status != self._get_previous_status(conn.id)):
                    self._get_previous_status.cache_clear() if hasattr(self._get_previous_status, 'cache_clear') else None
...
class ConnectionConfigDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.protocol_combo.addItems(["TCP客户端", "TCP服务端", "串口", "WebSocket", "HTTP", "Modbus TCP"])
    ...
    def test_connection(self):
        ...
        try:
            ...
            if self.connection_id:
                # 测试现有连接
                conn = self.connection_manager.get_connection(self.connection_id)
                if conn and conn.protocol_instance:
                    success = self._test_protocol_instance(conn.protocol_instance, config)
    ...
    def _create_and_test_protocol(self, protocol_type: str, config: Dict[str, Any]) -> bool:
        """创建协议实例并测试"""
        try:
            # 这里应该根据协议类型创建实际的协议实例
            # 暂时返回True，表示连接成功
    ...
    def _test_protocol_instance(self, protocol_instance, config: Dict[str, Any]) -> bool:
        """测试协议实例"""
        try:
            # 这里应该测试实际的协议实例
            # 暂时返回True
...
class CommunicationConfigWidget(QWidget):
    ...
    # 信号：连接状态变化
    connection_status_changed = pyqtSignal(str, str, bool)  # connection_id, status, is_connected
    ...
    def edit_connection(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if connection:
                ...
                # 发送连接更新信号
                self.connection_status_changed.emit(connection.id, connection.status, connection.is_connected)
    ...
    def on_connection_status_changed(self, connection: ProtocolConnection):
        ...
        for row in range(self.connection_table.rowCount()):
            if self.connection_table.item(row, 0).data(Qt.UserRole) == connection.id:
                # 更新状态
                ...
                # 更新统计
                self.connection_table.item(row, 3).setText(str(connection.send_count))
                self.connection_table.item(row, 4).setText(str(connection.receive_count))
                self.connection_table.item(row, 5).setText(str(connection.error_count))
                break
