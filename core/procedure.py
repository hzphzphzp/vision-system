...
class Procedure:
    ...
    def add_tool(self, tool: ToolBase, position: Tuple[int, int] = None) -> bool:
        """
        添加工具到流程
    ...
    def connect(self, from_tool: str, to_tool: str, 
                from_port: str = "OutputImage", to_port: str = "InputImage") -> bool:
        """
        连接两个工具
        ...
        from_port_exists = any(p.name == from_port for p in from_tool_obj.output_ports)
        to_port_exists = any(p.name == to_port for p in to_tool_obj.input_ports)
        
        if not from_port_exists:
            self._logger.error(f"源工具不存在输出端口: {from_tool}.{from_port}")
            return False
        ...
        self._logger.info(f"连接工具: {from_tool}.{from_port} -> {to_tool}.{to_port}")
    ...
    def get_connections_from(self, tool_name: str) -> List[ToolConnection]:
        ...
        Returns:
            连接列表
        """
        return [conn for conn in self._connections if conn.from_tool == tool_name]
    ...
    def get_connections_to(self, tool_name: str) -> List[ToolConnection]:
        ...
        Returns:
            连接列表
        """
        return [conn for conn in self._connections if conn.to_tool == tool_name]
        ...
        try:
            ...
            for tool_name in execution_order:
                ...
                if not has_connections and current_input is not None:
                    ...
                    self._logger.debug(f"为工具 {tool_name} 设置当前可用输入数据: {current_input.shape if current_input.is_valid else '无效'}")
                
                # 3. 检查工具是否有输入数据
                self._logger.debug(f"工具 {tool_name} 输入状态: 有输入={tool.has_input()}")
                ...
                try:
                    ...
                    if output is not None:
                        ...
                        self._logger.debug(f"工具 {tool_name} 输出: {output.shape if output.is_valid else '无效'}")
                    ...
                    self._logger.error(f"工具执行失败: {tool_name}, 错误: {str(e)}")
                    results[tool_name] = {
                        "error": str(e),
                        "result": None
                    }
            ...
            self._logger.info(f"流程执行完成: {self._name}, 耗时={self._execution_time*1000:.2f}ms")
            
            return results
            
        except Exception as e:
            self._last_error = str(e)
            self._logger.error(f"流程执行失败: {self._name}, 错误={self._last_error}")
    ...
    def copy(self) -> 'Procedure':
        ...
        for conn in self._connections:
            new_procedure.connect(conn.from_tool, conn.to_tool, 
