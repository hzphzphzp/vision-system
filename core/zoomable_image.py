...
class ZoomableGraphicsView(QGraphicsView):
    ...
    def __init__(self, scene: QGraphicsScene = None, parent: QWidget = None):
        ...
        self.setRenderHint(QPainter.RenderHint.SmoothPixmapTransform)  # 添加平滑像素变换
    ...
    def wheelEvent(self, event):
        ...
        zoom_factor = self._zoom_factor if delta > 0 else 1.0 / self._zoom_factor
    ...
    def set_zoom(self, zoom: float):
        ...
        if abs(new_zoom - self._zoom) > 0.001:
            ...
            self._logger.debug(f"[ZoomableGraphicsView] 缩放比例: {self._zoom:.2f}")
    ...
    def set_image_pixmap(self, pixmap: QPixmap):
        ...
        self._pixmap_item.setPos(rect.center() - self._pixmap_item.boundingRect().center())
        
        # 重置缩放
        self.reset_zoom()
        
        self._logger.debug(f"[ZoomableGraphicsView] 设置图像: {pixmap.width()}x{pixmap.height()}")
