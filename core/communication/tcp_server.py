...
class TCPServer(ProtocolBase):
    ...
    def receive(self, timeout: float = None) -> Tuple[Optional[str], Optional[Any]]:
        """不支持此方法"""
        logger.warning("[TCPServer] 请使用receive_from获取指定客户端数据")
        return None, None
    
    def receive_from(self, client_id: str = None, timeout: float = 0.1) -> Tuple[Optional[str], Optional[Any]]:
        """从指定客户端接收数据
    ...
    def _accept_loop(self):
        ...
        while self._running:
            try:
                # 检查连接数是否超过限制
                with self._clients_lock:
                    if len(self._clients) >= self._config.get("max_connections", 1000):
                        time.sleep(0.1)
                        continue
                ...
                with self._clients_lock:
                    ...
                    if len(self._clients) > self._statistics["max_connections"]:
                        self._statistics["max_connections"] = len(self._clients)
                
                # 使用线程池处理客户端
                if self._thread_pool:
                    self._thread_pool.submit(self._client_handler, client_id, client_socket, addr)
                else:
                    # 降级为线程处理
    ...
    def _client_handler(self, client_id: str, client_socket: socket.socket, addr: Tuple[str, int]):
        """客户端处理线程"""
        buffer = b""
        ...
        while self._running:
            try:
                ...
                if current_time - last_activity > self._config.get("connection_timeout", 60.0):
                    logger.warning(f"[TCPServer] 客户端 {client_id} 连接超时")
                    break
                
                # 接收数据
                data = client_socket.recv(self._config.get("receive_buffer_size", 4096))
                if not data:
                    break
                ...
                if self._running:
                    logger.error(f"[TCPServer] 客户端 {client_id} 接收异常: {e}")
...
if __name__ == "__main__":
    ...
    server = TCPServer()
    server.register_callback("on_client_connect", lambda cid, addr: print(f"客户端 {cid} 已连接: {addr}"))
    server.register_callback("on_client_disconnect", lambda cid: print(f"客户端 {cid} 已断开"))
