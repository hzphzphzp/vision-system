...
class HTTPClient(ProtocolBase):
    ...
    def connect(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"[HTTPClient] 会话已初始化: {self._base_url or '无基础URL'}")
            return True
    ...
    def post(self, url: str = "", data: Any = None, json: Dict = None, **kwargs) -> Dict:
        """POST请求"""
    ...
    def _request(self, method: str, url: str = "", **kwargs) -> Dict:
        ...
        full_url = f"{self._base_url}/{url.lstrip('/')}" if self._base_url and not url.startswith(("http://", "https://")) else url
    ...
    def upload(self, url: str, file_path: str, field_name: str = "file", 
        ...
        try:
            ...
            response = self._session.post(url, files=files, data=data, timeout=self._timeout)
    ...
    def receive(self, timeout: float = None) -> Any:
        """HTTP客户端不支持此方法"""
        logger.warning("[HTTPClient] 不支持receive方法，请使用get/post等HTTP方法")
        return None
