...
from core.communication.protocol_base import ProtocolBase, ProtocolType, ConnectionState
...
class ProtocolManager:
    ...
    def create_protocol(self, protocol_type: ProtocolType, name: str = None) -> ProtocolBase:
        """创建协议实例
        ...
        if protocol_id in self._protocols:
            logger.warning(f"[ProtocolManager] 协议 {protocol_id} 已存在，将返回现有实例")
            return self._protocols[protocol_id]
        
        protocol = self._create_protocol_instance(protocol_type)
        protocol.register_callback("on_error", lambda msg: logger.error(f"[{protocol.protocol_name}] {msg}"))
        
        with self._protocol_lock:
            self._protocols[protocol_id] = protocol
        
        logger.info(f"[ProtocolManager] 创建协议: {protocol_id} ({protocol_type.value})")
        return protocol
    
    def _create_protocol_instance(self, protocol_type: ProtocolType) -> ProtocolBase:
        """创建协议实例"""
