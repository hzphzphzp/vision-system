...
class ToolBase(ABC):
    ...
    # 默认输入输出端口
    INPUT_PORTS = [ToolPort("InputImage", "input", "image", "输入图像")]
    ...
    def __init__(self, name: str = None):
        ...
        self._logger = logging.getLogger(
            f"Tool.{self.tool_category}.{self.tool_name}"
        )
        ...
        if hasattr(self, "PARAM_DEFINITIONS") and self.PARAM_DEFINITIONS:
            for key, param_def in self.PARAM_DEFINITIONS.items():
                if hasattr(param_def, "default"):
                    ...
                    self._logger.debug(
                        f"初始化参数: {self._name}.{key} = {param_def.default}"
                    )
    ...
    def set_param(
        ...
        if fixed_value != value:
            self._logger.debug(
                f"参数 {key} 自动修正: {value} -> {fixed_value}"
            )
    ...
    def get_param_with_details(self) -> Dict[str, Dict[str, Any]]:
        ...
        for param_name, param_value in self._params.items():
            ...
            if param_name in param_definitions:
                ...
                option_labels = self._params.get(
                    f"__option_labels_{param_name}"
                )
    ...
    def set_input(self, image_data: ImageData, port: str = "InputImage"):
        ...
        self._logger.debug(
            f"设置输入: {port}, shape={image_data.shape if image_data.is_valid else 'None'}"
        )
        ...
        try:
            ...
            if not self._check_input():
                raise ToolException(
                    "输入数据无效",
                    error_code=400,
                    details={"tool": self._name, "check": "input"},
                )
            ...
            self._logger.info(
                f"执行完成: {self._name}, 耗时={self._execution_time*1000:.2f}ms"
            )
            ...
            self._result_data.message = get_error_message(
                400, self._last_error
            )
            ...
            # 使用错误管理模块记录错误
            log_error(
                400, f"{self._name}: {self._last_error}", {"tool": self._name}
            )
            ...
            raise ToolException(
                get_error_message(400, f"{self._name}: {self._last_error}"),
                error_code=400,
                details={"tool": self._name, "error": str(e)},
            )
            ...
            # 使用错误管理模块记录错误
            log_error(
                422, f"{self._name}: {self._last_error}", {"tool": self._name}
            )
            ...
            raise ToolException(
                get_error_message(422, f"{self._name}: {self._last_error}"),
                error_code=422,
                details={"tool": self._name, "error": str(e)},
            )
            ...
            # 使用错误管理模块记录错误
            log_error(
                502, f"{self._name}: {self._last_error}", {"tool": self._name}
            )
            ...
            raise ToolException(
                get_error_message(502, f"{self._name}: {self._last_error}"),
                error_code=502,
                details={"tool": self._name, "error": str(e)},
            )
            ...
            raise ToolException(
                get_error_message(500, f"{self._name}: {self._last_error}"),
                error_code=500,
                details=details,
            )
...
class ImageSourceToolBase(ToolBase):
    ...
    OUTPUT_PORTS = [ToolPort("OutputImage", "output", "image", "输出图像")]
...
class ToolRegistry:
    ...
    def get_tools_by_category(cls, category: str) -> Dict[str, Type[ToolBase]]:
        """获取指定类别的所有工具"""
        return {
            k: v for k, v in cls._tools.items() if k.startswith(f"{category}.")
        }
    ...
    @classmethod
    def create_tool(
        cls, category: str, name: str, *args, **kwargs
    ) -> ToolBase:
        """创建工具实例"""
