    ...
    def trigger(self, event_type: str, **kwargs):
        ...
        for callback in self._callbacks.get(event_type, []):
            try:
                ...
                logging.getLogger("SolutionCallback").error(f"回调执行失败: {e}")
...
class Solution:
    ...
    def is_running(self) -> bool:
        """获取运行状态"""
        return self._state in [SolutionState.RUNNING, SolutionState.CONTINUOUS_RUN]
    ...
    def run(self, input_data: ImageData = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.info(f"方案运行完成: {self._name}, 耗时={self._execution_time*1000:.2f}ms")
            ...
            self._logger.error(f"方案运行失败: {self._name}, 错误={self._last_error}")
            
            # 触发错误事件
            self._callback.trigger("run_error", data={"error": self._last_error})
    ...
    def __repr__(self) -> str:
        return (f"Solution(name={self._name}, "
