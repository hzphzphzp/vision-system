...
from core.tool_base import VisionAlgorithmToolBase, ToolParameter
...
class ROIToolMixin:
    ...
    def get_roi_from_params(self, image_width: int = None, image_height: int = None) -> Optional[Tuple[int, int, int, int]]:
        """从参数中获取ROI区域
    ...
    def extract_roi_region(self, image) -> Optional:
        ...
        if roi:
            roi_x, roi_y, roi_width, roi_height = roi
            return image[roi_y:roi_y + roi_height, roi_x:roi_x + roi_width].copy()
        return None
