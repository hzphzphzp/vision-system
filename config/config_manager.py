...
# 配置文件路径
DEFAULT_CONFIG_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "configs")
...
class ConfigManager:
    ...
    def _load_config(self):
        ...
        if os.path.exists(DEFAULT_CONFIG_FILE_JSON):
            try:
                with open(DEFAULT_CONFIG_FILE_JSON, "r", encoding="utf-8") as f:
    ...
    def validate_config(self) -> bool:
        ...
        try:
            # 验证必要的配置项
            required_sections = ["system", "camera", "image_processing", "paths"]
            ...
            for path_key, path_value in paths.items():
                if not os.path.isabs(path_value):
                    # 相对路径，转换为绝对路径
                    abs_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), path_value)
