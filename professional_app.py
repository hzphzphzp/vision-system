...
from tools.template_match import GrayMatch, LineFind, CircleFind
...
class ProfessionalVisionApp:
    ...
    def add_image_source(self, name: str, source_type: str = "local", 
                          file_path: str = None) -> ImageSource:
        """
        添加图像源
    ...
    def add_camera_source(self, name: str, camera_id: str = "0") -> CameraSource:
        """
        添加相机源
    ...
    def add_filter(self, name: str, filter_type: str = "gaussian",
                   kernel_size: int = 3) -> object:
        """
        添加滤波工具
    ...
    def add_gray_match(self, name: str, template_path: str,
                       min_score: float = 0.7) -> GrayMatch:
        """
        添加灰度匹配工具
    ...
    def add_circle_find(self, name: str, min_radius: int = 10,
                        max_radius: int = 100) -> CircleFind:
        """
        添加圆查找工具
    ...
    def connect_tools(self, from_name: str, to_name: str,
                      from_port: str = "OutputImage", 
                      to_port: str = "InputImage") -> bool:
        """
        连接两个工具
        ...
        """
        if self.current_procedure:
            return self.current_procedure.connect(from_name, to_name, 
                                                  from_port, to_port)
        return False
            ...
            if isinstance(proc_results, dict):
                for tool_name, tool_result in proc_results.items():
                    if isinstance(tool_result, dict):
                        ...
                        if output is not None and isinstance(output, ImageData):
                            logger.info(f"  {tool_name}: 图像大小={output.width}x{output.height}")
    ...
    def create_demo_solution(self) -> Solution:
        ...
        # 添加图像源
        image_source = ToolRegistry.create_tool("ImageSource", "图像读取器", "图像源")
        image_source.set_param("source_type", "local")
        procedure.add_tool(image_source)
        
        # 添加高斯滤波
        gaussian = ToolRegistry.create_tool("ImageFilter", "高斯滤波", "高斯滤波")
...
def test_tools():
    ...
    # 创建工具实例
    image_source = ToolRegistry.create_tool("ImageSource", "图像读取器", "测试图像源")
    print(f"\n创建工具: {image_source}")
