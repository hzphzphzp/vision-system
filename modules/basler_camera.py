...
from modules.camera_adapter import CameraAdapter, CameraInfo, CameraType, CameraState, TriggerMode
...
class BaslerCameraAdapter(CameraAdapter):
        ...
        try:
            ...
            if camera is None:
                if self._camera_id.startswith("basler_"):
                    ...
                    if idx < len(devices):
                        camera = pylon.TlFactory.GetInstance().CreateDevice(devices[idx])
                    else:
                        camera = pylon.TlFactory.GetInstance().CreateDevice(devices[0])
                else:
                    camera = pylon.TlFactory.GetInstance().CreateDevice(devices[0])
            ...
            self._logger.info(f"Basler相机连接成功: {model_name} (SN: {serial_number})")
            return True
    ...
    def _grab_loop(self):
        ...
        while not self._stop_event.is_set() and self._is_grabbing:
            try:
                grab_result = self._camera.RetrieveResult(1000, pylon.TimeoutHandling_ThrowException)
    ...
    def set_trigger_mode(self, mode: str) -> bool:
        ...
        try:
            ...
            if mode == "continuous":
                ...
                self._camera.TriggerSource.SetValue(pylon.TriggerSource_Software)
...
# 注册Basler适配器
from modules.camera_adapter import CameraAdapterFactory, CameraType
