...
from utils.exceptions import CameraConnectionException, CameraCaptureException, CameraException
...
def _import_mv_camera_control():
    ...
    try:
        from MvCameraControl_class import MvCamera, MV_CC_DEVICE_INFO_LIST, MV_CC_DEVICE_INFO, MV_GIGE_DEVICE, MV_USB_DEVICE, MV_FRAME_OUT
        from CameraParams_header import MVCC_INTVALUE, MVCC_FLOATVALUE, MVCC_ENUMVALUE, MVCC_STRINGVALUE
...
class HikCamera:
    ...
    def connect(self) -> bool:
        ...
        try:
            from ctypes import byref, memset, sizeof, cast, c_ubyte
    ...
    def _get_device_info(self, sdk):
        ...
        try:
            ...
            ret = self._camera.MV_CC_GetStringValue("DeviceModelName", stDeviceName)
            if ret == sdk['MV_OK']:
                model_name = ''.join([chr(c) for c in stDeviceName.chCurValue if c != 0])
    ...
    def set_trigger_mode(self, mode: str) -> bool:
        ...
        try:
            if mode == "continuous":
                ...
                if ret != 0:
                    self._logger.warning(f"设置触发源为Software失败: 0x{ret:x}")
                    return False
    ...
    def get_one_frame(self, timeout_ms: int = 1000) -> Optional[np.ndarray]:
        ...
        try:
            from ctypes import byref, sizeof, POINTER, c_ubyte, memset, cast
    ...
    def get_all_parameter_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ret = self._camera.MV_CC_GetFloatValue("ExposureTime", stFloatParam)
            if ret == sdk['MV_OK']:
            ...
            ret = self._camera.MV_CC_GetFloatValue("AcquisitionFrameRate", stFps)
            if ret == sdk['MV_OK']:
...
class CameraManager:
    ...
    def discover_devices(self) -> List[CameraInfo]:
        ...
        try:
            ...
            for i in range(device_list.nDeviceNum):
                mvcc_dev_info = cast(device_list.pDeviceInfo[i], POINTER(sdk['MV_CC_DEVICE_INFO'])).contents
                ...
                if mvcc_dev_info.nTLayerType == sdk['MV_GIGE_DEVICE']:
                    ...
                    self._camera_list.append(CameraInfo(
                        id=camera_id,
                        name=display_name,
                        model=model_name,
    ...
    def connect(self, camera_id: str) -> Optional[HikCamera]:
        ...
        try:
            ...
            stDeviceInfo = cast(device_info, POINTER(sdk['MV_CC_DEVICE_INFO'])).contents
