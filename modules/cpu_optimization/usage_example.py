...
def example_parallel_processing():
    ...
    data = list(range(20))
    start = time.time()
    results = batch_process(data, process_func=process_item, batch_size=5, show_progress=True)
    elapsed = time.time() - start
...
def example_batch_detection():
    ...
    detector = create_yolo26_detector(
        "models/yolov8n.onnx",
        CPUInferenceConfig(num_threads=4)
    )
    ...
    if detector.is_loaded:
        ...
        for i, image in enumerate(images):
            ...
            print(f"  图像 {i+1}: {len(result.boxes)} 个目标, {result.inference_time_ms:.2f}ms")
