...
from .models.yolo26_cpu import YOLO26CPUDetector
from .api.cpu_detector import CPUDetectorAPI, DetectionResult
from .utils.performance_monitor import PerformanceMonitor, PerformanceMetrics, get_performance_monitor
...
def print_system_info():
    ...
    print(f"CPU核心数: {capabilities['cpu_count']} (物理: {capabilities.get('physical_cpu_count', 'N/A')})")
    print(f"Python版本: {capabilities['python_version']}")
    print(f"总内存: {capabilities['memory_info'].get('total', 'N/A') / (1024**3):.2f} GB")
    print("SIMD支持:", ", ".join([f for f in capabilities['simd_support'] if f.startswith(('sse', 'avx', 'neon'))]) or "无检测到")
    print("-" * 60)
    print("可用优化功能:")
