...
class CPUDetectorAPI:
    ...
    def set_config(self, **kwargs):
        ...
        """
        allowed_params = {
            'confidence_threshold', 'nms_threshold', 'num_threads', 
            'input_size', 'max_batch_size'
        }
    ...
    def load_model(self, model_path: str, 
        ...
        success = self._detector.load_model(
            model_path, 
            class_names_path if class_names_path else None
        )
    ...
    def warmup(self, num_iterations: int = 5) -> bool:
        ...
        try:
            ...
            self._trigger_callback("on_warmup_complete", {"iterations": num_iterations})
            return True
    ...
    def detect(self, image: np.ndarray,
              ...
              return_image: bool = False) -> DetectionResult:
        """
        检测图像
    ...
    def detect_from_path(self, image_path: str,
                        draw_results: bool = False) -> DetectionResult:
        """
        从文件路径检测图像
        ...
        if not os.path.exists(image_path):
            return DetectionResult(
                success=False,
                error_message=f"文件不存在: {image_path}"
            )
    ...
    def _update_performance_stats(self, inference_time: float):
        ...
        avg_time = self._performance_stats["total_time"] / self._performance_stats["total_inferences"]
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        """获取性能统计"""
        return {
            "last_inference_time_ms": self._performance_stats.get("last_inference_time", 0),
            "total_inferences": self._performance_stats.get("total_inferences", 0),
            "avg_inference_time_ms": self._performance_stats.get("avg_inference_time", 0),
...
# 便捷函数
def create_detector_api(model_path: str,
                       config: APIConfig = None) -> CPUDetectorAPI:
    """
    创建CPU检测器API的便捷函数
