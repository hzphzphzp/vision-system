...
class TestToolRegistry:
    ...
    def test_tool_creation_from_registry(self, tool_registry):
        ...
        tool = tool_registry.create_tool(
            "ImageFilter", "高斯滤波", "gaussian_test"
        )
        assert tool is not None
    ...
    def test_invalid_tool_creation(self, tool_registry):
        ...
        with pytest.raises(ValueError):
            tool_registry.create_tool(
                "InvalidCategory", "InvalidTool", "invalid_test"
            )
...
class TestProcedure:
    ...
    def test_add_remove_tool(self, sample_procedure, tool_registry):
        ...
        # 创建一个工具
        tool = tool_registry.create_tool(
            "ImageFilter", "高斯滤波", "gaussian_tool"
        )
    ...
    def test_tool_connections(self, sample_procedure, tool_registry):
        """测试工具连接"""
        # 创建两个工具
        tool1 = tool_registry.create_tool(
            "ImageFilter", "高斯滤波", "gaussian_tool"
        )
    ...
    def test_execution_order(self, sample_procedure, tool_registry):
        ...
        tool2 = tool_registry.create_tool("Vision", "直线查找", "line_tool")
        tool3 = tool_registry.create_tool(
            "Recognition", "二维码识别", "qr_tool"
        )
        ...
        assert execution_order.index("gaussian_tool") < execution_order.index(
            "line_tool"
        )
        assert execution_order.index("line_tool") < execution_order.index(
            "qr_tool"
        )
