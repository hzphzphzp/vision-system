...
class TestImageStitching(unittest.TestCase):
    ...
    def _load_test_images(self) -> list:
        ...
        try:
            ...
            cv2.putText(img1, "Image 1", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            ...
            cv2.putText(img2, "Image 2", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            ...
            cv2.putText(img3, "Image 3", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    def test_different_feature_detectors(self):
        ...
        for detector in detectors:
            try:
                ...
                print(f"检测器 {detector}: {'成功' if result.status else '失败: ' + result.message}")
            except Exception as e:
    ...
    def test_different_blend_methods(self):
        ...
        for method in blend_methods:
            try:
                ...
                # 即使匹配失败，也应该返回结果对象
                self.assertIsInstance(result, type(self.stitcher.process([])))
                print(f"融合方法 {method}: {'成功' if result.status else '失败: ' + result.message}")
            except Exception as e:
