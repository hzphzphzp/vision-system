...
def _apply_box_filter(image: np.ndarray, kernel_size: int = 3, normalize: bool = True) -> np.ndarray:
    """方框滤波"""
...
def _apply_gaussian_filter(image: np.ndarray, kernel_size: int = 3, 
                           sigma_x: float = 0, sigma_y: float = 0) -> np.ndarray:
    """高斯滤波"""
    return cv2.GaussianBlur(image, (kernel_size, kernel_size), sigma_x, sigma_y)


def _apply_median_filter(image: np.ndarray, kernel_size: int = 3) -> np.ndarray:
    """中值滤波"""
    return cv2.medianBlur(image, kernel_size)


def _apply_bilateral_filter(image: np.ndarray, diameter: int = 9,
                            sigma_color: float = 75, sigma_space: float = 75) -> np.ndarray:
    """双边滤波"""
    return cv2.bilateralFilter(image, diameter, sigma_color, sigma_space)


def apply_morphology(image: np.ndarray, operation: str, 
    ...
    return cv2.morphologyEx(image, morph_operations[operation], kernel, iterations=iterations)


def validate_kernel_size(kernel_size: int, param_name: str = "kernel_size") -> int:
    """验证并修正核大小
