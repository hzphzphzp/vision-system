...
class RecoveryManager:
    ...
    def __init__(self):
        ...
        self._recovery_history: List[Tuple[ErrorContext, RecoveryStatus, Optional[str]]] = []
    ...
    def register_strategy(self, error_type: str, strategy: RecoveryAction):
        ...
        with self._lock:
            ...
            self._logger.debug(f"注册恢复策略: {error_type} -> {strategy.strategy.value}")
    ...
    def recover(self, error_context: ErrorContext) -> RecoveryStatus:
        ...
        Returns:
            恢复状态
        """
        self._logger.info(f"开始错误恢复: {error_context.error_type}, 组件={error_context.component}")
        ...
        try:
            ...
            if not strategy:
                ...
                if not strategy:
                    self._logger.warning(f"未找到恢复策略: {error_context.error_type}")
    ...
    def _execute_strategy(self, strategy: RecoveryAction, 
                         error_context: ErrorContext) -> RecoveryStatus:
        """
        执行恢复策略
    ...
    def _execute_retry_strategy(self, strategy: RecoveryAction, 
                               error_context: ErrorContext) -> RecoveryStatus:
        """
        执行重试策略
        ...
        Returns:
            恢复状态
        """
        self._logger.info(f"执行重试策略: 最多{strategy.max_attempts}次, 延迟{strategy.delay}秒")
        
        for attempt in range(strategy.max_attempts):
            self._logger.info(f"重试尝试 {attempt + 1}/{strategy.max_attempts}")
    ...
    def _execute_fallback_strategy(self, strategy: RecoveryAction, 
                                  error_context: ErrorContext) -> RecoveryStatus:
        """
        执行降级策略
    ...
    def _execute_restart_strategy(self, strategy: RecoveryAction, 
                                 error_context: ErrorContext) -> RecoveryStatus:
        """
        执行重启策略
    ...
    def _execute_ignore_strategy(self, strategy: RecoveryAction, 
                                 error_context: ErrorContext) -> RecoveryStatus:
        """
        执行忽略策略
    ...
    def _execute_alert_strategy(self, strategy: RecoveryAction, 
                               error_context: ErrorContext) -> RecoveryStatus:
        """
        执行告警策略
    ...
    def _record_recovery(self, error_context: ErrorContext, 
    ...
    def get_recovery_history(self, limit: int = 100) -> List[Tuple[ErrorContext, RecoveryStatus, Optional[str]]]:
        """
        获取恢复历史
...
class SystemRecovery:
    ...
    def recover_from_error(self, error_type: str, error_code: int, 
                          error_message: str, component: str, 
                          details: Dict[str, Any]) -> RecoveryStatus:
        """
        从错误中恢复
    ...
    def register_custom_strategy(self, error_type: str, strategy: RecoveryAction):
        """
        注册自定义恢复策略
...
def recover_from_error(error_type: str, error_code: int, 
                      error_message: str, component: str, 
                      details: Dict[str, Any] = None) -> RecoveryStatus:
    """
    从错误中恢复
