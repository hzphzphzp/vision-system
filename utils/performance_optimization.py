...
class DataProcessor:
    ...
    @staticmethod
    def process_detection_results(results: List[Dict[str, Any]]) -> Optional[pl.DataFrame]:
        """
    ...
    @staticmethod
    def filter_by_confidence(df: pl.DataFrame, min_confidence: float = 0.5) -> pl.DataFrame:
        """
        根据置信度过滤结果
    ...
    def compute_statistics(df: pl.DataFrame) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_confidence": df.select(pl.mean("confidence")).item() if "confidence" in df.columns else 0,
            "unique_classes": df.select(pl.col("class")).n_unique() if "class" in df.columns else 0
        }
        return stats
...
def benchmark_data_processing():
    ...
    # 生成测试数据
    test_data = [
        {"class": f"class_{i % 10}", "confidence": np.random.random(), "x": np.random.randint(0, 640), "y": np.random.randint(0, 480), "width": np.random.randint(10, 100), "height": np.random.randint(10, 100)}
